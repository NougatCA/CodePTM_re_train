...
from models import build_or_load_completion_line_model
from utils import get_filenames, get_elapse_time, load_and_cache_gen_data, get_special_tokens
...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria, accelerator=None):
    ...
    pred_fn = os.path.join(args.output_dir, "predictions.txt")

    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2, open(pred_fn, "w") as f3:
        for pred_nl, gold in zip(pred_nls, eval_examples):

            f.write(pred_nl.strip() + '\n')
            f1.write(gold.target.strip() + '\n')
            f2.write(gold.source.strip() + '\n')
            f3.write(pred_nl.strip() + "\n")

    result = {'em': 0, 'bleu': 0, "codeblue": 0.0}
...
def main():
    ...
    args.special_tokens = get_special_tokens(lit_file)

    config, model, tokenizer = build_or_load_completion_line_model(args)
